...
export function RecapCard({ content, recapData, currency = "INR" }: RecapCardProps) {
  ...
  return (
    <div className="w-full rounded-[8px] border border-fiber-line bg-card-bg p-4 sm:p-5 shadow-xs space-y-3.5 text-xs text-ink-text">
      {/* Header Badge */}
      <div className="flex items-center justify-between border-b border-fiber-line pb-2.5">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-[4px] bg-stamp-indigo/10 text-stamp-indigo">
        ...
        <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-[3px] border border-stamp-indigo/30 bg-stamp-indigo/5 text-stamp-indigo font-bold">
          Proactive AI
        </span>
      </div>
      ...
      {stats && projection && (
        <div className="rounded-[6px] border border-fiber-line bg-paper-bg p-3 grid grid-cols-2 sm:grid-cols-3 gap-2 text-center font-mono">
          ...
          <div className="space-y-0.5">
            ...
            <p className="font-bold text-stamp-indigo text-xs sm:text-sm">
      ...
      {(topPattern || topOpportunity) && (
        <div className="space-y-1.5 pt-0.5">
          {topPattern && (
            <div className="flex items-start gap-2 text-xs text-muted-text bg-card-bg border border-fiber-line/80 rounded-[4px] p-2.5">
              <TrendingUp className="h-4 w-4 text-passbook-gold shrink-0 mt-0.5" />
              <div>
          ...
          {topOpportunity && (
            <div className="flex items-start gap-2 text-xs text-muted-text bg-card-bg border border-fiber-line/80 rounded-[4px] p-2.5">
              <Lightbulb className="h-4 w-4 text-stamp-emerald shrink-0 mt-0.5" />
              <div>
      ...
      <div className="pt-1 flex justify-end">
        <Link
          href="/analysis"
          className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-stamp-indigo hover:underline hover:opacity-85 transition-opacity"
        >
